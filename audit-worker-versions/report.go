...
func sectionTotalLabel(title string) string {
	...
	case "Unresponsive worker pools":
...
func renderReadmeAt(snapshot WorkerSnapshot, reportGeneratedAt time.Time) string {
	...
	sections := [5]reportSection{
		...
		generateReadmeSection("Unknown implementation", "These pools claimed and resolved the probe task, but did not publish a recognized worker log artifact. Their worker implementation and version could therefore not be identified.", workers, func(w WorkerInfo) bool { return w.hasNoArtifacts }),
